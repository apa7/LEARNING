...
public class Problem4 {
    ...
    //pos1: nums1的起始位置 pos2: nums2的起始位置
    public int findKth(int[] nums1, int pos1, int[] nums2, int pos2, int k) {
        if (pos1 >= nums1.length) return nums2[pos2 + k - 1];//nums1为空数组
        if (pos2 >= nums2.length) return nums1[pos1 + k - 1];//nums2为空数组
        if (k == 1) {
    ...
    public static void main(String[] args) {
        ...
        random.nextInt(5);
        double r = p.findMedianSortedArrays(new int[]{1, 2}, new int[]{3, 4});
